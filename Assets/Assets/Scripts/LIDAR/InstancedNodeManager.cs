...
namespace Player.LIDAR {
    public class InstancedNodeManager : MonoBehaviour {
        // manage nodes shader. for efficiency, however it's only marginal
        ...
        void Awake() {
            ...
            if (instancedMaterial is not null && !instancedMaterial.enableInstancing) {
                instancedMaterial.enableInstancing = true;
            }
        }

        public void AddInstance(Vector3 position, Color color) {
            // add a node to what's being managed
            matrices.Add(Matrix4x4.TRS(position, Quaternion.identity, Vector3.one * 0.0075f));
            colors.Add(color);
        }
